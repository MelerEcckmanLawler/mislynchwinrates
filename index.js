...
function doStuff(report, reportId) {
  ...
  townLynched = new Array(12).fill(0)
  townWon = false
  for (let i = 0; i < entries.length; i++) {
    let entry = entries[i]
    if (entry.type == 'TRIAL OUTCOME') {
      if (entry.outcome == 'lynched') {
        if (players[entry.accused].faction == 'Town') {
          townLynched[entry.time.split('.')[0][1]] = 1
        }
      }
    }
  ...
  console.log(townLynched.reduce((a, b) => a + b, 0) + ' ' + townWon)

  townLynched = townLynched.reduce((a, b) => a + b, 0)

  OUTPUT[townLynched] = OUTPUT[townLynched] || {}
  OUTPUT[townLynched][townWon] = ++OUTPUT[townLynched][townWon] || 1
